...
@Component({
  selector: 'app-recipts-new',
  templateUrl: './recipts-new.component.html',
  styleUrls: ['./recipts-new.component.scss'],
  providers: [ConfirmationService,MessageService]
})
export class ReciptsNewComponent {
    ...
    this._piSub = sah.saveReceipt(newInvoice).subscribe({
      ...
      next:(v) => {
        ...
        if (v.hasOwnProperty('error')) {
          ...
          this.loadInvoices(0,0)
          this.loadPartyBalances(0,0)
          this.router.navigate(['account/recipts'])
          return;
        }
        else if(v == null) {
