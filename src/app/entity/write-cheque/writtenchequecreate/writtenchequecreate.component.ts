...
@Component({
  selector: 'app-writtenchequecreate',
  templateUrl: './writtenchequecreate.component.html',
  styleUrls: ['./writtenchequecreate.component.scss'],
  providers: [ConfirmationService, MessageService],
})
export class WrittenchequecreateComponent {
  ...
  filterBankParties(event: any) {
      ...
      let criteria: Search = <Search>{
          ...
          screen: 'tokenfield',
          offset: 0,
          searchtype: 'bank-party-accounthead-contains',
      };
      console.log('CRITERIA', criteria);
      let pService: AccountHeadListService = new AccountHeadListService(
          this.httpClient
      ...
      this._piSub = sah.saveCheque(cheque).subscribe({
          ...
          next: (v) => {
              ...
              if (v.hasOwnProperty('error')) {
                  ...
                  this.loadCheques(0, 0);
                  this.router.navigate(['entity/writeCheque']);
                  return;
              } else if (v == null) {
                  this.inProgress = false;
                  this.confirm(
